...
\newcommand{\bbnote}[1]{\color{red} \emph{#1} \color{black}}
\date{\today}
\begin{document}
...
\begin{itemize}
\item on CRAN: \code{aod} (overdispersed models such as beta-binomial);
...
Plot the profiles:
<<profplot1,fig=TRUE,height=5,width=10>>=
par(mfrow=c(1,2))
...
Data are incorporated in the \code{aod} package:
<<aod>>=
library(aod)
summary(orob1)
@ 
...
<<crowdertab,echo=FALSE,results=tex>>=
...
(m1B <- mle2(y~dbetabinom(prob,size=n,theta),
...
<<gg1plot,fig=TRUE>>=
print(gg1 + geom_line(data=pdat1,colour="red")+
...
<<basegraphprofplot,fig=TRUE>>=
print(plot(prof4))
@ 

(2) Using \code{xyplot} from the \code{lattice} package:
\setkeys{Gin}{width=\textwidth}
<<latticeprof,fig=TRUE,height=5,width=10>>=
...
<<ggplotprof,fig=TRUE,height=5,width=10>>=
  ...
  \item{controls on the profiling (which can be set manually, but for which there
    ...
    \begin{itemize}
      ...
      \item \code{del} Step size (scaled by standard error) (Default: \code{zmax}/5.)
        \bbnote{This seems reasonable, but I don't know where it comes from.}
    ...
    \item{Stop the profiling: 
      \begin{itemize}
        ...
        \item if $z$ is \code{NA} (\verb+stop_na+) \bbnote{(for greater robustness, should
          we try to keep going anyway?)}
...
\begin{itemize}
\item If necessary, construct the profile
...
It is also possible to use \code{method="quad"} (this is obsolete, should
be replaced by documentation pointing to \code{confint.default}) and
\code{method="uniroot"} (undocumented for now; rather than approximating
from the profile, try to use \code{uniroot} with calls to the negative
log-likelihood function to identify the confidence intervals as precisely
and robustly as possible (should be more accurate, but slower --- must
also be computed for each specified $\alpha$ level separately).
