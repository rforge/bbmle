...
\subsection*{\code{MuMIn} package (Kamil Barto{\'n}), r-forge}

<<keep.source=TRUE>>=
library(MuMIn)
## MuMIn has only QAIC, not QAICc -- add it
QAICc <- function(object,...,chat,nobs) {
  df <- attr(logLik(object),"df")+1
  QAIC(object,...,chat=chat)+2*df*(df+1)/(nobs-df-1)
}
(gg <- dredge(glmOT.D93,rank="QAIC",chat=dfun(glmOT.D93)))
(ggc <- dredge(glmOT.D93,rank="QAICc",nobs=length(counts),chat=dfun(glmOT.D93)))
detach("package:MuMIn")
@ 

Since \code{dredge} is clever, can work with quasi- models as well by
using \code{deviance()}:
<<eval=FALSE>>=
(ggqc <- dredge(glmQOT.D93,rank="QAICc",nobs=length(counts),
